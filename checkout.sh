#!/usr/bin/env bash

echo "git clone git@github.com:statisticsnorway/data-collector-api.git"
git clone git@github.com:statisticsnorway/data-collector-api.git

echo "git clone git@github.com:statisticsnorway/data-collector-server-base.git"
git clone git@github.com:statisticsnorway/data-collector-server-base.git

echo "git clone git@github.com:statisticsnorway/data-collector-testutils.git"
git clone git@github.com:statisticsnorway/data-collector-testutils.git

echo "git clone git@github.com:statisticsnorway/data-collector-connector-content-stream-rawdata.git"
git clone git@github.com:statisticsnorway/data-collector-connector-content-stream-rawdata.git

echo "git clone git@github.com:statisticsnorway/data-collector-javascript-processor.git"
git clone git@github.com:statisticsnorway/data-collector-javascript-processor.git

echo "git clone git@github.com:statisticsnorway/data-collector-core.git"
git clone git@github.com:statisticsnorway/data-collector-core.git

echo "git clone git@github.com:statisticsnorway/data-collector-docker.git"
git clone git@github.com:statisticsnorway/data-collector-docker.git

echo "git clone git@github.com:statisticsnorway/data-collector-cron-docker.git"
git clone git@github.com:statisticsnorway/data-collector-cron-docker.git

echo "git clone git@github.com:statisticsnorway/data-collector-samples.git"
git clone git@github.com:statisticsnorway/data-collector-samples.git

echo "git clone git@github.com:statisticsnorway/data-collection-consumer-specifications.git"
git clone git@github.com:statisticsnorway/data-collection-consumer-specifications.git

